...
data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'Data', 'GoodReads')
...
def load_books_to_read(catalog):
    ...
    """
    to_read_file = os.path.join(data_dir, 'to_read.csv')
...
def get_books_stack_by_user(catalog, user_id):
    ...
    for pos in range(lt.size(catalog['books_to_read'])):
        book_to_read = lt.get_element(catalog['books_to_read'], pos)
